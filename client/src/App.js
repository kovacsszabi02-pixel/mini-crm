...
const STATUSES = [
  ...
  '11. Sablonírás', '12. Átadásra vár', '13. Teljesített', '14. Elveszített'
];

function AddPartnerForm({ onPartnerAdded }) {
  const [formData, setFormData] = useState({
    company_name: '', contact_person: '', phone: '', email: '', revenue: '', tax_number: '', billing_address: '', manager: '', accepted_offer: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setFormData({ company_name: '', contact_person: '', phone: '', email: '', revenue: '', tax_number: '', billing_address: '', manager: '', accepted_offer: '' });
        onPartnerAdded();
      }
...
function App() {
  ...
  const handleSelectPartner = (partner) => {
    ...
    setStatusNote(''); setPlainNote(''); setNewTaskDesc(''); setNewDocName(''); setNewDocUrl('');
  ...
  const handleAddTask = async () => {
    ...
    const res = await fetch(`${BACKEND_URL}/api/partners/${selectedPartner.id}/tasks`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: newTaskDesc })
    });
    if (res.ok) { setNewTaskDesc(''); fetchTasks(selectedPartner.id); fetchLogs(selectedPartner.id); }
  };
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                {tasks.map(t => (
                  <li key={t.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px', background: '#fff', padding: '8px', borderRadius: '4px', border: '1px solid #ccc' }}>
                    ...
                    <span style={{ textDecoration: t.is_completed ? 'line-through' : 'none', color: t.is_completed ? '#6c757d' : '#000' }}>{t.description}</span>
                  </li>
                ))}
              </ul>
            </div>
